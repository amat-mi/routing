...
WITH grafo AS (
  SELECT id, version, user_id, tstamp, changeset_id, tags, nodes, bbox, linestring as geom, modo, varcosto
  FROM adb.web_000_grafo_costi 
)

SELECT
      id, node_id, dist, pos, geom_line, verifica ,modo,  varcosto,
     ROW_NUMBER() OVER(PARTITION BY id,modo ORDER BY dist) AS rownum
  FROM 
  (
    SELECT
      ...
      a.geom AS geom_line, c.geom AS geom_point,a.modo,  a.varcosto,
...
CREATE TABLE adb.t_nodi_fine AS (
  ...
  SELECT 
    a.id, a.node_id, a.dist,a.modo, a.varcosto,
    CASE 
...
(SELECT 
    ...
    a.rownum AS rownum_max, a.modo, a.varcosto
  FROM 
    (SELECT  id, MAX(pos)pos, MAX(rownum) rownum, modo, varcosto  FROM adb.t_nodi GROUP BY  id,modo, varcosto) a, 
    (SELECT id, MAX(pos)pos, MAX(rownum) rownum, modo FROM adb.t_nodi_fine GROUP BY modo,  id) b
  WHERE
...
(
  -- nodi_fine
  SELECT  id, node_id, dist, pos, rownum, geom_line, modo, varcosto
  FROM adb.t_nodi
  ...
  SELECT
     a.id, a.node_id, a.dist, 1 AS pos, (rownum_max+1) AS rownum, geom_line, a.modo, a.varcosto
  FROM
    adb.t_nodi_fine a,
    adb.t_linea_chiusa b
  WHERE
    a.id=b.id and a.modo = b.modo AND linea_chiusa=1 AND pos=0
);
...
CREATE TABLE adb.t_spezza_archi AS (
  SELECT 
    ROW_NUMBER() OVER(ORDER BY a.id, a.modo) AS id, a.id AS way_originale, a.node_id AS node_start, b.node_id AS node_end, a.rownum as rownum1, b.rownum as rownum2,
    ST_LineSubstring(a.geom_line, a.pos, b.pos) AS geom   ,a.modo, a.varcosto
  FROM 
...
WITH grafo AS (
  ...
  SELECT 
     id, version, user_id, tstamp, changeset_id, tags, nodes, bbox, geom, modo,varcosto,
...
SELECT 
   ...
   b.version, b.tags, b.bus, b.oneway, b.reverse,
    a.geom, a.modo, a.varcosto
FROM
  adb.t_spezza_archi a JOIN adb.t_bus_oneway b ON a.way_originale=b.id and a.modo = b.modo
